...
int32_t text_7bit_encode(const char *txt_in, char **txt_out) {
  ...
  const uint32_t out_length =
      in_length >= 8 ? encode_get_outId_from_inId(in_length * 8) / 8 : in_length;
...
int32_t text_7bit_decode(const char *txt_in, char **txt_out) {
  ...
  // todo: add check if user provides unexpected count of bytes, which can not
  // be properly decoded, according to rules
  const uint32_t out_length =
      in_length >= 7 ? decode_get_outId_from_inId(in_length * 8) / 7 : in_length;
